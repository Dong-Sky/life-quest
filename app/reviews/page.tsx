...
export default function ReviewsPage() {
  ...
  const redeemedThisWeek = state.redemptions.filter((redemption) => getPrototypeWeekKey(new Date(redemption.redeemedAt)) === weekKey);
  ...
  return <div className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
    ...
    <div className="mt-7 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
      <section className="rounded-xl border border-[var(--line)] bg-white p-5">
        <div><h2 className="text-base font-semibold">本周战报</h2><p className="mt-1 text-xs text-[var(--muted)]">从 {summary.weekKey} 开始的真实任务结算。</p></div>
        {settledQuests.length ? <div className="mt-5 divide-y divide-[var(--line)]">{settledQuests.map((quest) => <article className="flex items-center justify-between gap-3 py-3" key={quest.id}><div className="min-w-0"><p className="truncate text-sm font-medium">{quest.title}</p><p className="mt-1 text-xs text-[var(--muted)]">已完成结算</p></div><p className="shrink-0 text-sm text-[var(--muted)]">+{quest.reward?.xp ?? 0} XP · +{quest.reward?.coins ?? 0} coins</p></article>)}</div> : <p className="mt-5 rounded-lg bg-[var(--surface)] px-4 py-5 text-sm leading-6 text-[var(--muted)]">本周还没有结算任务。完成第一项任务后，这里会留下你的战报。</p>}
        <div className="mt-6 border-t border-[var(--line)] pt-5"><h3 className="text-sm font-semibold">本周已兑换奖励</h3>{redeemedThisWeek.length ? <div className="mt-3 space-y-2">{redeemedThisWeek.map((redemption) => <div className="flex items-center justify-between rounded-lg bg-[var(--surface)] px-3 py-2.5 text-sm" key={redemption.id}><span>{redemption.rewardName}</span><span className="text-[var(--success)]">−{redemption.coinCost} coins</span></div>)}</div> : <p className="mt-2 text-sm text-[var(--muted)]">本周还没有兑换奖励。</p>}</div>
      </section>

      <section className="rounded-xl border border-[var(--line)] bg-white p-5">
        <h2 className="text-base font-semibold">下周三件事</h2><p className="mt-1 text-xs leading-5 text-[var(--muted)]">每一项都会以完整任务形式创建：可设置类型、难度、重要度、心理阻力与可选主线。</p>
        {plan ? <div className="mt-5"><div className="rounded-lg bg-[var(--surface)] p-4"><p className="text-sm font-medium">已创建 {plannedQuests.length} 项下周任务</p>{plannedQuests.length ? <ul className="mt-3 space-y-2 text-sm text-[var(--muted)]">{plannedQuests.map((quest) => <li key={quest?.id}>· {quest?.title}</li>)}</ul> : null}</div><Link className="mt-4 inline-flex rounded-lg bg-[var(--ink)] px-4 py-2 text-sm font-medium text-white" href="/quests">前往任务清单</Link></div> : <div className="mt-5 space-y-4">{priorities.map((priority, index) => <fieldset className="rounded-lg border border-[var(--line)] p-3" key={index}><legend className="px-1 text-xs text-[var(--muted)]">第 {index + 1} 件</legend><input className="mt-1 w-full rounded-lg border border-[var(--line)] px-3 py-2.5 text-sm outline-none focus:border-[var(--accent)]" onChange={(event) => updatePriority(index, { title: event.target.value })} placeholder="下一步要推动的具体行动" value={priority.title} /><div className="mt-3 grid gap-2 sm:grid-cols-2"><Select label="任务类型" options={fieldOptions.questType} value={priority.questType} onChange={(value) => updatePriority(index, { questType: value as QuestType })} /><Select label="难度" options={fieldOptions.difficulty} value={priority.difficulty} onChange={(value) => updatePriority(index, { difficulty: value as Difficulty })} /><Select label="重要度" options={fieldOptions.importance} value={priority.importance} onChange={(value) => updatePriority(index, { importance: value as Importance })} /><Select label="心理阻力" options={fieldOptions.resistance} value={priority.resistance} onChange={(value) => updatePriority(index, { resistance: value as Resistance })} /></div><label className="mt-3 grid gap-1.5 text-xs text-[var(--muted)]"><span>所属主线（可选）</span><select className="rounded-lg border border-[var(--line)] bg-white px-2.5 py-2 text-sm text-[var(--ink)]" onChange={(event) => updatePriority(index, { mainlineId: event.target.value || undefined })} value={priority.mainlineId ?? ""}><option value="">暂不关联主线</option>{state.mainlines.map((mainline) => <option key={mainline.id} value={mainline.id}>{mainline.name}</option>)}</select></label></fieldset>)}<button className="rounded-lg bg-[var(--ink)] px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-40" disabled={!priorities.some((priority) => priority.title.trim())} onClick={createPlan} type="button">创建下周任务</button></div>}
      </section>
    </div>
  </div>;
}
