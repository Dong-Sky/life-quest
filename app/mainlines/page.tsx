  ...
  return <div className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
    ...
    <section className="mt-7 space-y-3">{state.mainlines.length ? state.mainlines.map((mainline) => {
      ...
      return <article className="rounded-xl border border-[var(--line)] bg-white p-5" key={mainline.id}>
        ...
        <section className="mt-5 border-t border-[var(--line)] pt-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-medium">关联副本 · 有终点的项目</p><p className="mt-1 text-xs text-[var(--muted)]">把这条长期方向拆成一个个可完成的项目。</p></div><Link className="text-xs font-medium text-[var(--accent)] hover:underline" href="/projects">管理副本 →</Link></div>{relatedProjects.length ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{relatedProjects.map((project) => { const projectTasks = state.quests.filter((quest) => quest.projectId === project.id); const done = projectTasks.filter((quest) => quest.status === "completed").length; return <Link className="rounded-lg border border-[var(--line)] bg-[#fafafa] px-3 py-3 transition hover:border-[var(--accent)] hover:bg-white" href="/projects" key={project.id}><div className="flex items-start justify-between gap-2"><p className="text-sm font-medium">{project.name}</p><span className="shrink-0 text-xs text-[var(--muted)]">{done}/{projectTasks.length}</span></div><p className="mt-2 text-xs text-[var(--muted)]">{project.victoryCondition || "尚未写下胜利条件"}</p></Link>; })}</div> : <p className="mt-3 rounded-lg border border-dashed border-[var(--line)] bg-[#fafafa] px-3 py-3 text-xs text-[var(--muted)]">还没有关联副本。需要一个明确终点时，创建副本并把它归入这条主线。</p>}</section>
