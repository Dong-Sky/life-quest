...
export default function FriendsPage() {
  ...
  const shareUrl = inviteCode ? `${window.location.origin}/friends?invite=${inviteCode}` : "";
  const incoming = new URLSearchParams(typeof window === "undefined" ? "" : window.location.search).get("invite");
  ...
  return <div className="mx-auto max-w-3xl px-5 py-8 sm:px-8">
    ...
    {status === "ready" ? <div className="mt-6 space-y-6">
      ...
      <section className="rounded-xl border border-[var(--line)] bg-white p-5"><p className="text-sm font-semibold">邀请一位搭档</p><p className="mt-1 text-sm leading-6 text-[var(--muted)]">复制链接发给老婆或朋友。对方注册并打开链接后，双方确认即可成为好友。</p><div className="mt-4 flex flex-col gap-3 sm:flex-row"><input className="min-w-0 flex-1 rounded-lg border border-[var(--line)] bg-[#fafafa] px-3 py-2.5 text-xs text-[var(--muted)]" readOnly value={shareUrl} /><button className="shrink-0 rounded-lg bg-[var(--ink)] px-4 py-2 text-sm font-medium text-white" onClick={() => void copyInvite()} type="button">复制邀请链接</button></div></section>

      <section><div className="flex items-center justify-between"><h2 className="text-base font-semibold">我的搭档</h2><span className="text-xs text-[var(--muted)]">{friends.filter((friend) => friend.status === "accepted").length} 位好友</span></div><div className="mt-3 space-y-3">{friends.length ? friends.map((friend) => <article className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[var(--line)] bg-white p-4" key={friend.friendship_id}><div><p className="font-medium">{friend.display_name}</p><p className="mt-1 text-xs text-[var(--muted)]">{friend.status === "accepted" ? "已成为搭档" : friend.direction === "received" ? "等待你确认" : "已发送邀请，等待对方确认"}</p></div>{friend.status === "pending" && friend.direction === "received" ? <button className="rounded-md bg-[var(--accent-soft)] px-3 py-2 text-xs font-medium text-[var(--accent)]" onClick={() => void accept(friend.friendship_id)} type="button">确认好友</button> : null}</article>) : <div className="rounded-xl border border-dashed border-[var(--line)] bg-white p-7 text-center"><p className="font-medium">还没有搭档。</p><p className="mt-2 text-sm text-[var(--muted)]">先复制上方邀请链接，发给想一起推进生活副本的人。</p></div>}</div></section>
