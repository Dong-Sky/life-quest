...
export default function RewardsPage() {
  ...
  const create = () => {
    const next = createPrototypeReward(state, { name, coinCost, isRepeatable });
    if (next !== state) {
      save(next);
      setName("");
      setCoinCost(50);
  ...
  return <div className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
    ...
    <form className="mt-5 rounded-xl border border-[var(--line)] bg-[#fafafa] p-4" onSubmit={(event) => { event.preventDefault(); create(); }}><div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_180px]"><label className="grid gap-2 text-sm"><span className="font-medium">想兑换什么？</span><input className="rounded-lg border border-[var(--line)] bg-white px-3 py-2.5 outline-none focus:border-[var(--accent)]" onChange={(event) => setName(event.target.value)} placeholder="例如：吃一顿喜欢的餐厅" value={name} /></label><label className="grid gap-2 text-sm"><span className="font-medium">所需金币</span><input className="rounded-lg border border-[var(--line)] bg-white px-3 py-2.5 outline-none focus:border-[var(--accent)]" min="1" onChange={(event) => setCoinCost(Number(event.target.value))} type="number" value={coinCost} /></label></div><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><label className="flex cursor-pointer items-center gap-2 text-sm"><input checked={isRepeatable} className="h-4 w-4 accent-[var(--accent)]" onChange={(event) => setIsRepeatable(event.target.checked)} type="checkbox" />可重复兑换</label><button className="rounded-lg bg-[var(--ink)] px-4 py-2 text-sm font-medium text-white" type="submit">创建奖励</button></div></form>
