  ...
  return <div className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
    ...
    <section className="mt-7 space-y-3">{state.projects.length ? state.projects.map((project) => {
      ...
      return <article className="rounded-xl border border-[var(--line)] bg-white p-5" key={project.id}>
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-semibold">{project.name}</p><p className="mt-2 text-sm text-[var(--muted)]">{project.victoryCondition || "尚未写下胜利条件。"}</p><p className="mt-3 text-xs text-[var(--muted)]">{mainline ? <Link className="font-medium text-[var(--accent)] hover:underline" href="/mainlines">主线 · {mainline.name}</Link> : "未归入长期方向"}{project.dueDate ? ` · 目标日期：${project.dueDate}` : ""}</p></div><div className="flex items-center gap-2"><div className="min-w-36 rounded-lg bg-[var(--accent-soft)] px-3 py-2 text-right text-xs text-[var(--accent)]"><strong>{complete} / {related.length}</strong> 个行动<br /><span>任务数量进度 · {percent}%</span></div><button className="rounded-md border border-[var(--line)] px-2.5 py-1.5 text-xs hover:bg-gray-50" onClick={() => startEdit(project)} type="button">编辑</button></div></div>
        ...
        <section className="mt-5 border-t border-[var(--line)] pt-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-medium">阶段里程碑 · 阶段成果</p><p className="mt-1 text-xs text-[var(--muted)]">{completeMilestones} / {milestones.length} 已达成 · 每个阶段的关联任务全部结算后自动完成</p></div><button className="rounded-md border border-[var(--line)] px-2.5 py-1.5 text-xs hover:bg-gray-50" onClick={() => { setAddingMilestoneTo(project.id); setMilestoneTitle(""); setMilestoneQuestIds([]); }} type="button">+ 新增里程碑</button></div>{addingMilestoneTo === project.id ? <MilestoneEditor title={milestoneTitle} questIds={milestoneQuestIds} tasks={related} onCancel={() => setAddingMilestoneTo(null)} onSave={(quickTaskTitle) => createMilestone(project.id, quickTaskTitle)} onTitleChange={setMilestoneTitle} onQuestIdsChange={setMilestoneQuestIds} saveLabel="添加里程碑" /> : null}{milestones.length ? <div className="mt-4 border-l-2 border-[var(--line)] pl-4">{milestones.map((milestone) => { const progress = getPrototypeMilestoneProgress(state, milestone); const isMilestoneEditing = editingMilestoneId === milestone.id; return <div className="relative mb-3 rounded-lg border border-[var(--line)] bg-white px-3 py-3 last:mb-0" key={milestone.id}><span aria-hidden="true" className={"absolute -left-[1.45rem] top-5 h-3 w-3 rounded-full border-2 border-white " + (progress.isCompleted ? "bg-[var(--success)]" : "bg-[var(--accent)]")} /><div className="flex items-center justify-between gap-3"><div><p className={progress.isCompleted ? "text-sm text-[var(--muted)] line-through" : "text-sm"}>{milestone.title}</p><p className="mt-1 text-xs text-[var(--muted)]">{progress.total ? `已结算 ${progress.completed} / ${progress.total} 项关联任务` : "尚未关联任务"}</p></div><div className="flex items-center gap-2">{progress.isCompleted ? <span className="text-xs text-[var(--success)]">自动完成</span> : <span className="text-xs text-[var(--muted)]">推进中</span>}<button className="rounded-md border border-[var(--line)] px-2.5 py-1.5 text-xs hover:bg-gray-50" onClick={() => startMilestoneEdit(milestone)} type="button">编辑</button></div></div>{isMilestoneEditing ? <MilestoneEditor title={editMilestoneTitle} questIds={editMilestoneQuestIds} tasks={related} onCancel={() => setEditingMilestoneId(null)} onSave={saveMilestoneEdit} onTitleChange={setEditMilestoneTitle} onQuestIdsChange={setEditMilestoneQuestIds} saveLabel="保存修改" /> : null}</div>; })}</div> : <p className="mt-3 rounded-lg border border-dashed border-[var(--line)] bg-[#fafafa] px-3 py-3 text-xs text-[var(--muted)]">还没有阶段。可以先新增一个阶段，再在其中直接创建或编入任务。</p>}</section>
