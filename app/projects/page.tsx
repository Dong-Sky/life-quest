...
export default function ProjectsPage() {
  ...
  const createMilestone = (projectId: string) => {
    const next = createPrototypeMilestone(state, projectId, milestoneTitle, milestoneQuestIds);
    if (next !== state) {
      save(next);
      setMilestoneTitle("");
  ...
  const saveMilestoneEdit = () => {
    ...
    const next = updatePrototypeMilestone(state, editingMilestoneId, editMilestoneTitle, editMilestoneQuestIds);
    if (next !== state) {
      save(next);
...
function MilestoneEditor({ tasks, title, questIds, onTitleChange, onQuestIdsChange, onSave, onCancel, saveLabel }: { tasks: PrototypeQuest[]; title: string; questIds: string[]; onTitleChange: (value: string) => void; onQuestIdsChange: (value: string[]) => void; onSave: () => void; onCancel: () => void; saveLabel: string }) {
  ...
  return <form className="mt-3 rounded-lg bg-[#fafafa] p-3" onSubmit={(event) => { event.preventDefault(); onSave(); }}><label className="grid gap-1.5 text-xs text-[var(--muted)]"><span>里程碑名称</span><input autoFocus className="rounded-lg border border-[var(--line)] bg-white px-2.5 py-2 text-sm text-[var(--ink)]" onChange={(event) => onTitleChange(event.target.value)} placeholder="例如：完成第一周训练安排" value={title} /></label><div className="mt-3"><p className="text-xs text-[var(--muted)]">关联任务（全部结算后自动完成）</p>{tasks.length ? <div className="mt-2 max-h-40 space-y-1 overflow-y-auto rounded-lg border border-[var(--line)] bg-white p-2">{tasks.map((task) => <label className="flex cursor-pointer items-center gap-2 rounded px-1.5 py-1.5 text-sm hover:bg-gray-50" key={task.id}><input checked={questIds.includes(task.id)} className="h-4 w-4 accent-[var(--accent)]" onChange={() => toggleQuest(task.id)} type="checkbox" /><span className={task.status === "completed" ? "text-[var(--muted)] line-through" : ""}>{task.title}</span>{task.status === "completed" ? <span className="ml-auto text-xs text-[var(--success)]">已结算</span> : null}</label>)}</div> : <p className="mt-2 rounded-lg border border-dashed border-[var(--line)] bg-white p-3 text-xs text-[var(--muted)]">这个副本暂时没有任务。先在任务页创建并关联任务，之后可回来编辑里程碑。</p>}</div><div className="mt-3 flex gap-2"><button className="rounded-md bg-[var(--ink)] px-3 py-2 text-xs font-medium text-white" type="submit">{saveLabel}</button><button className="rounded-md border border-[var(--line)] px-3 py-2 text-xs" onClick={onCancel} type="button">取消</button></div></form>;
}
