  ...
  return <div className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
    ...
    <section className="mt-7 overflow-hidden rounded-xl border border-[var(--line)] bg-white">
      {state.quests.map((quest) => {
        ...
        const milestone = state.milestones.find((item) => item.questIds.includes(quest.id));
        const hierarchy = milestone ? [mainline?.name, project?.name, milestone.title].filter(Boolean).join(" › ") : project ? [mainline?.name, project.name].filter(Boolean).join(" › ") : mainline?.name;
        ...
        return <div className="border-b border-[var(--line)] last:border-b-0" key={quest.id}>
          <article className="flex items-center justify-between gap-4 px-4 py-4"><div><p className={quest.status === "completed" ? "font-medium text-[var(--muted)] line-through" : "font-medium"}>{quest.title}</p><p className="mt-1 text-xs text-[var(--muted)]">{hierarchy ? "路径 · " + hierarchy : "未归入主线或副本"}{quest.dueDate ? " · DDL：" + quest.dueDate : ""}</p><p className="mt-1 text-xs text-[var(--muted)]">{recurring ?? `${quest.questType} · ${quest.difficulty}`}{milestone ? " · 阶段任务" : project ? " · 副本任务" : mainline ? " · 主线任务" : " · 独立任务"}</p></div><div className="flex shrink-0 items-center gap-2">{quest.status === "completed" ? <span className="text-right text-xs text-[var(--success)]">{quest.recurrence ? "本周期已完成" : `+${quest.reward?.xp} XP · +${quest.reward?.coins} coins`}</span> : <button className="rounded-md bg-[var(--accent-soft)] px-2.5 py-1.5 text-xs font-medium text-[var(--accent)]" onClick={() => save(settlePrototypeQuest(state, quest.id))}>{action}</button>}<button className="rounded-md border border-[var(--line)] px-2.5 py-1.5 text-xs hover:bg-gray-50" onClick={() => startEdit(quest)} type="button">编辑</button></div></article>
