...
export default function DashboardPage() {
  ...
  return <div className="mx-auto max-w-7xl px-5 py-7 sm:px-8 lg:py-9">
    ...
    <section className="mt-6 grid gap-5 lg:grid-cols-2">
      <FocusCollection title="正在推进的主线" href="/mainlines" linkText="查看主线" empty="先创建一条主线，把每天的行动连接到长期方向。" hasItems={state.mainlines.length > 0}>{state.mainlines.slice(0, 2).map((mainline) => { const count = state.quests.filter((quest) => quest.mainlineId === mainline.id && quest.status === "open").length; return <article className="min-h-[108px] rounded-xl bg-[#f8fbff] p-3.5" key={mainline.id}><p className="text-sm font-semibold">{mainline.name}</p><p className="mt-1 text-xs text-[var(--muted)]">{count} 个待推进任务</p></article>; })}</FocusCollection>
      <FocusCollection title="正在推进的副本" href="/projects" linkText="管理副本" empty="创建一个有终点的副本，并把具体任务关联进来。" hasItems={state.projects.length > 0}>{state.projects.slice(0, 2).map((project) => { const progress = getPrototypeProjectProgress(state, project.id); return <article className="min-h-[108px] rounded-xl bg-[#f8fbff] p-3.5" key={project.id}><div className="flex justify-between gap-3"><p className="min-w-0 truncate text-sm font-semibold">{project.name}</p><span className="shrink-0 text-xs font-semibold text-[var(--accent)]">{progress.percent}%</span></div><p className="mt-1 text-xs text-[var(--muted)]">{progress.completedXp}/{progress.plannedXp} XP 权重</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-[var(--accent)]" style={{ width: progress.percent + "%" }} /></div></article>; })}</FocusCollection>
    </section>
  </div>;
}
...
function TodayQuest({ quest, state, index, onComplete }: { quest: PrototypeQuest; state: PrototypeState; index: number; onComplete: (id: string) => void }) {
  ...
  return <article className="flex items-center gap-3 rounded-xl border border-[var(--line)] p-3.5 transition hover:border-blue-200 hover:bg-[#fbfdff]">
    <span aria-hidden="true" className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[var(--accent-soft)] text-sm font-semibold text-[var(--accent)]">{index}</span>
    <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{quest.title}</p><p className="mt-1 truncate text-xs text-[var(--muted)]">{context}</p></div>
    <div className="flex shrink-0 items-center gap-3"><div className="hidden text-right sm:block"><p className="text-sm font-semibold">{reward.xp} XP</p><p className="mt-1 text-xs text-[var(--gold)]">{reward.coins} coins</p></div><button aria-label={"完成并结算：" + quest.title} className="rounded-lg border border-[var(--accent)] bg-white px-3 py-2 text-xs font-semibold text-[var(--accent)] transition hover:bg-[var(--accent)] hover:text-white" onClick={() => onComplete(quest.id)} type="button">完成</button></div>
  </article>;
}
