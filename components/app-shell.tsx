...
export function AppShell({ children, accountEmail, onSignOut }: { children: ReactNode; accountEmail?: string; onSignOut?: () => void }) {
  ...
  return <div className="min-h-screen bg-[var(--canvas)] lg:grid lg:grid-cols-[248px_minmax(0,1fr)]">
    <aside className="border-b border-[var(--line)] bg-white px-3 py-4 lg:min-h-screen lg:border-b-0 lg:border-r lg:px-4">
      ...
      {accountEmail ? <div className="mt-5 rounded-xl border border-[var(--line)] bg-[#fbfbfb] p-3"><p className="truncate text-xs text-[var(--muted)]" title={accountEmail}>{accountEmail}</p>{onSignOut ? <button className="mt-2 text-xs font-medium text-[var(--muted)] transition hover:text-[var(--ink)]" onClick={onSignOut} type="button">退出登录</button> : null}</div> : null}
    </aside>
